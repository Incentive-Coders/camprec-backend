...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      SecPassword = await bcryptjs.hash(password, salt);
...
router.post("/edit", async (req, res) => {
  ...
  var data = await StudentSchema.findByIdAndUpdate(student_id, {
    name: name,
    about: about,
    date_of_birth: date_of_birth,
    resume: resume,
...
router.post("/addedu", async (req, res) => {
  ...
  var data = await StudentSchema.findByIdAndUpdate(student_id, {
    $push: {
      education: { course: course, institute: institute, marks: marks },
    },
  });
  res.send(data);
});

router.post("/addcer", async (req, res) => {
...
router.post("/delskill", async (req, res) => {
  let { student_id, skill } = req.body;
  var data = await StudentSchema.findByIdAndUpdate(student_id, {
    $pull: { skills: skill },
  });
...
router.post("/delexp", async (req, res) => {
  let { student_id, names, companys } = req.body;
  var data = await StudentSchema.findByIdAndUpdate(student_id, {
    $pull: { experience: { names: names, companys: companys } },
  });
  res.send(data);
});

router.post("/deledu", async (req, res) => {
  let { student_id, course, institute } = req.body;
  var data = await StudentSchema.findByIdAndUpdate(student_id, {
    $pull: { education: { course: course, institute: institute } },
  });
  res.send(data);
});
router.post("/delcer", async (req, res) => {
  let { courses, institutes, student_id } = req.body;
  var data = await StudentSchema.findByIdAndUpdate(student_id, {
    $pull: { certification: { courses: courses, institutes: institutes } },
  });
  res.send(data);
});

router.post("/jaccept", async (req, res) => {
...
router.get("/approves/:page", async (req, res) => {
  var page = req.params.page;
  console.log("list");
  page = (page - 1) * 10;
  const data = await StudentSchema.find(
...
router.get("/premium/:page", async (req, res) => {
  var page = req.params.page;
  console.log("list");
  page = (page - 1) * 10;
  const data = await StudentSchema.find(
...
router.post("/showjob", async (req, res) => {
